...
public class BinaryTree<E> {
     ...
     * Setters
     */
    public void setLeft(BinaryTree<E> child){this.left = child;}
    public void setRight(BinaryTree<E> child){this.right = child;}
    ...
    public int height(){
        int height = 0;
        if (isLeaf()){ return 0; }
        if (hasLeft()) {height = Math.max(height, left.height());}
        if (hasRight()) {height = Math.max(height, right.height());}
        return height + 1; // inner: one higher than highest child
    }
    ...
    public boolean equals(BinaryTree<E> t2){
        ...
        if (data != t2.data) return false;
        if (hasLeft() && !getLeft().equals(t2.getLeft())) return false;
        if (hasRight() && !getRight().equals(t2.getRight())) return false;
        return true;
    }
