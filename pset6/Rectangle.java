...
public class Rectangle implements Shape {
     ...
     * @param y1 top y coord
     * @param x2 right x coord
     * @param y2 bottom y coord
	...
	public void moveBy(int dx, int dy) {
        // Move the rectangle by dx in x direction and dy in y direction
        x1 += dx;
        y1 += dy;
        x2 += dx;
        y2 += dy;
	...
	public void draw(Graphics g) {
        g.setColor(color);
        g.fillRect(x1, y1, x2 - x1, y2 - y1); // Draw filled rectangle
	}
